...
def excluir_contato():
    ...
    if messagebox.askyesno("Confirmar", f"Tem certeza que deseja excluir o contato {contatos[id_contato]['nome']}?"):
    ...
    if termo:
        ...
        for id_contato, contato in contatos.items():
            if termo in contato['nome'].lower():
                tree.insert("", tk.END, values=(id_contato,
                    contato['nome'],
...
tk.Button(button_frame, text="Todos os Contatos", width=20, command=mostrar_todos_contatos).grid(row=0, column=5, padx=5)
def preencher_campos_selecionados(event):
    selecionado = tree.selection()
    print("Item selecionado:", selecionado)  # Debug: Verifique se há seleção
    if selecionado:
        valores = tree.item(selecionado)['values']
        print("Valores do item:", valores)  # Debug: Verifique os dados
...
tree.column("WhatsApp", width=150)
tree.column("LinkedIn", width=200)
tree.column("GitHub", width=200)
